...
void ListeReservation(char* filename,int id_parking, Date date){
    ...
    //FILE* result = fopen("ListeReserv.txt", "w");
    Reservation reservation;
    while(fscanf(file, "%d, %49[^,], %49[^,], %49[^,], %d, %d, %d, %d/%d/%d, %d/%d/%d\n",
            ...
            &reservation.arr_jour, &reservation.arr_mois, &reservation.arr_heure,
            &reservation.dep_jour, &reservation.dep_mois, &reservation.dep_heure) == EOF){

        if(reservation.ID_Parking == id_parking && date.jour == reservation.arr_jour && date.mois == reservation.arr_mois){
            printf("Une reservation pour %s %s", reservation.nom, reservation.prenom);
        }
    }
    fclose(file);
}
